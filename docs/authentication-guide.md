...
OAuth2 模式下暂不展示“切换会话”。当前 `/threads` 列表来自密语模式的本地身份配置，不代表 OAuth2 provider 的授权身份；后续需要先定义 OAuth2 身份与 CLAWeb 会话/room 的绑定关系，再接入新的切换逻辑。

session 结构、WS 握手、历史记录加载及聊天收发逻辑在两种模式下保持一致。
...
| `CLAWEB_UI_SEND_MODE` | `enter` | 默认发送方式：`enter` 或 `modifier`（Ctrl/Shift/Cmd+Enter 发送） |
